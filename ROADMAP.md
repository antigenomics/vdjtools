...
| 15 | `feature/model-workshop` | **Model workshop**: `from_germline` (build on ANY V(D)J library; `from_arda` becomes a wrapper, output byte-identical) + FASTA reader + germline audit; `check_model` (tidy issue frame, DP-derived deletion reachability); `score` (nt-Pgen log-likelihood, AIC/BIC, support-based `k`, Pgen distribution comparison, scenario/sequence entropy + Hill q=1/q=2 diversity); `compare_models`/`compare_net_dot`; persisted EM training log; `infer_frame`; `extend_alleles` (gene-usage-preserving); `data.build_all` (parallel corpus build); `vdjtools model` CLI sub-app; `docs/model.rst`; `examples/model_workshop.py`. Released GIL on `pgen_nt` (11.7× threaded, bitwise-identical). **v3.3.0** | **done** |
