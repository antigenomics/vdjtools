...
| ⚠ **Prepared clonotype subsets — API exists, artifact NOT YET UPLOADED** | intended path `prepared/{group}.{CHAIN}.{label}.tsv.gz` in the same HF dataset | gzipped TSV, the columns `data.unique_clonotypes` emits (`v_call j_call junction junction_aa locus d_call d2_call count`); ~100k unique clonotypes, ~2 MB each | **Produce and publish** (needs arda + mmseqs2 + HF write access), then commit through the local mirror `~/hf/airr_model_read` — never the API (§6):<br>`df = data.prepare(group, chain, "nonfunctional", out_dir=...)`<br>`df.head(100_000).write_csv(mirror/f"prepared/{group}.{chain}.nonfunctional.tsv.gz", separator="\t")`<br>Read back with `vdjtools.model.data.load_prepared(group, chain, label)` | **experimental** (a subset of the rows above, not a new measurement). Intended for the `model_workshop` notebook and HF-gated tests, so neither needs arda/mmseqs2. `load_prepared` raises a `FileNotFoundError` naming `data.prepare` until the files exist; nothing in the test suite or the notebook depends on them (both fall back to simulated sequences) |
