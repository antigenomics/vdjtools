...
| Real AIRR reads (model training, all 7 loci) | **private** HF dataset [`isalgo/airr_model_read`](https://huggingface.co/datasets/isalgo/airr_model_read) (owner, cc-by-nc) | AIRR/native clonotype tables (`.parquet`/`.tsv.gz`) | `vdjtools.model.data.load_reads(...)` (lazy `huggingface_hub`, needs a HF token; not redistributed) | **experimental**; out-of-frame reads that train the models (EM) and will exercise D-D. **Being populated** — schema/filenames finalize on arrival; the loader + `test_model_reads.py` skip until then |
