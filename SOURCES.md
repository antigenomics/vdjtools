...
| Real AIRR reads (model training, all 7 loci) | HF dataset [`isalgo/airr_model_read`](https://huggingface.co/datasets/isalgo/airr_model_read) (owner, cc-by-nc-nd) | raw 5'-RACE **FASTQ** `{group}/{CHAIN}.{functional,nonfunctional}.fq.gz`; groups = `human`, `human_fetal` (TdT-low), `mouse` | `vdjtools.model.data.prepare(group, chain, label)` = fetch (lazy `huggingface_hub`) → `arda rnaseq map` (V/D/**D2**/J + junction + productivity) → `unique_clonotypes` (dedup to `(v,j,junction)`). Whole-corpus, several chains in parallel: `vdjtools model build --chains TRB,TRA,IGH --workers 4` (= `vdjtools.model.data.build_all`) | **experimental**; out-of-frame reads train the models (EM), functional reads are the selection test set. D-D annotated per-read via `d2_call`. `yields.tsv` records per-bucket read counts |
| **Prepared clonotype examples (human TRA + TRB), in-repo** | derived from the `human/{TRA,TRB}.nonfunctional.fq.gz` row above | gzipped **FASTA**, `tests/python/fixtures/model_reads/human.{TRA,TRB}.nonfunctional.fa.gz`. Header `>{i}\|{v_call}\|{j_call}\|{d_call}\|{d2_call}\|{count}`, sequence = junction nt. TRB 100,000 records / 1.54 MB, TRA 34,238 / 0.47 MB | Regenerate: `reads = data.annotate_reads(fq, out_dir=…, prefix=…)` → `uniq = data.unique_clonotypes(reads)` → `data.write_prepared(uniq.head(100_000), path)`. Read: `data.load_prepared("human", "TRB")` | **derived/computed** from the experimental reads above — arda 2.19 `arda map` (stage 1 only), then dedup to `(v_call, j_call, junction)`. Yields: TRB 167,582 reads → 123,030 unique clonotypes (capped to 100k); TRA 61,332 → 34,238 (all kept). Ships in the source tree, **not** in the wheel (`sdist.exclude` drops `/tests`), so `load_prepared` is a checkout-only convenience for the `model_workshop` notebook and the tests. ~0.01% of junctions carry an ambiguous base; both training entry points substitute `A` by default and warn (`ambiguous=None` drops instead) |
...
| **Influenza (`airr_flu_vac`)** — the weak/negative control | HF [`isalgo/airr_flu_vac`](https://huggingface.co/datasets/isalgo/airr_flu_vac) | 17 × `samples/<id>.airr.tsv.gz` + `metadata.tsv` (19 cols, **CRLF**; `v_call`/`j_call` **gene-level only**, no allele) | **ONE donor (`YB`)** — no cohort-level analysis is possible; seasons 13/14/16 are the longitudinal axis; timepoints `pre0`/`0`/`5`/`12`/`45` (day −14/0/5/12/45); F1/F2 replicates in seasons 14+16 only; **UMI-corrected** `duplicate_count`; class I HLA (one genotype) | **experimental**. Sycheva AL et al., *Vaccine* 2018;36(12):1599–1605, [10.1016/j.vaccine.2018.02.027](https://doi.org/10.1016/j.vaccine.2018.02.027), PMID 29454515. SRA SRP111073. NOTE: **paywalled, no PMC copy** — the abstract reports "several" new memory clonotypes peaking day 45 and gives no method or count, so **there is no verified numeric ground truth** for this cohort |
| **YF-specific TCR reference** (the enrichment validation target) | VDJdb via `vdjmatch.db.load(asset="slim", gene="TRB")`, filtered `epitope == "LLWNGPMAV"` (the loader **normalises** VDJdb's dotted names: `epitope`, `mhc_a`, `reference_id` — not `antigen.epitope`). NB `vdjmatch.db.fetch_hf` **defaults to `repo="isalgo/airr_benchmark"`** — an upstream provenance dependency this file did not previously record | TSV | **2,138 unique TRB CDR3aa**, A\*02:01/NS4b-restricted, stratified by DONOR: **2,071 donor-disjoint** + **67 donor-matched** (PMID:30459272) | **100% tetramer/dextramer-sorted** across all 8 studies — `method.identification` in VDJdb's **full** dump (the slim asset drops the column); **zero** computationally-inferred entries. NOTE: **A previous revision of this row claimed "409 unique … five independent studies … no Pogorelyy paper, no twin cohort" and excluded PMID:30459272 + PMID:32081129 as circular. That was wrong and is RETRACTED.** Pogorelyy 2018 PNAS produced two things under one PMID — responding TCRs called from repertoire *dynamics* (which would be circular) and a *tetramer validation assay* (which is not); VDJdb ingested the second. And Minervina eLife 2020 (PMID:32081129, 1,675 records = 77%) is donor-DISJOINT: it studied donors M1/P30, and the twin codes P1/P2/Q1/Q2/S1/S2 appear nowhere in its full text. Excluding both discarded **81% of the reference for a leak that does not exist**. Only the 67 from PMID:30459272 share donors with airr_yfv19 (= that paper's twin cohort); they are kept as an **ascertainment diagnostic** (a tetramer sort of these donors near peak preferentially recovers clones already abundant at d15, so that stratum should read stronger). NOTE: `mhc_a` resolution is inconsistent (`HLA-A*02:01` vs `HLA-A*02`) — the epitope already pins the restriction; `vdjdb_score` is 0 for 2,120/2,138 (VDJdb's default for high-throughput sorting without secondary verification) so it does **not** discriminate here |

NOTE: **`airr_benchmark/alice/yf/{P1..S2}_{d0,d15}.tsv.gz` is NOT a reference set.** It is the ALICE paper's
...
NOTE: **`airr_yfv19` traps, all verified against the actual metadata rather than the paper:**
...
NOTE: **`airr_tbev_vac`**: gate on `reads`, **not** on `library=main` — `IZ_reactive_IFNgpos` is `main` at
...
|---|---|---|---|---|---|---|---|---|---|
| covid19 (COVID vs healthy) | **502+ / 34−** NOTE: 15:1 | 10 | 52,528 | **0** | none to validate (OR=nan) — 34 controls cannot power a genome-wide screen | — | — | — | 142 s / 6.4 GB |
...
NOTE: **covid19 association is an honest negative: 0 significant on 502 COVID+ vs 34 healthy.** The FMBA
...
NOTE: **covid19_vacc (timepoint) is withdrawn for a different and unfixed reason: the design.** The 269
...
NOTE: candidate features capped at `max_features=2000` (warned at runtime), i.e. top-incidence pairs only.
...
**NOTE: Correction.** An earlier revision of this file argued the signal was *"not a depth artifact —
...
NOTE: **Earlier revisions of this file reported 2,805 / 502 / "82%" and three different tested counts
...
| `arda` (9 models: 7 human loci + **mouse TRA/TRB**) | the same native EM as `learned`, but seeded from the `from_arda` scaffold rather than the OLGA model, so the gene set + germline come from **arda** (`vdjtools.model.reference`) | `python appendix/build_bundled_models.py` | **computed from experimental data**, in the **arda IMGT allele namespace**. **Unaffected by the J-anchor defect** — arda's `germline_nt` is already the CDR3 region, so `derive_orf` never runs — which is what made it a valid reference for sizing that bug. Use this set when the rest of the pipeline is arda-annotated (e.g. `mirpy`'s prototypes and baked germline distances) so generated sequences share one allele namespace with the query data instead of falling back on name matching; it is also the only bundled set with a non-human organism. NB `from_arda` on its own returns *placeholder* marginals meant to be refit — these are the refit ones. |
...
## Generated reference tables (shipped in the wheel)

| Dataset | Origin | Format | How to regenerate | Provenance |
|---|---|---|---|---|
| **Adaptive/immunoSEQ → IMGT gene map** `python/vdjtools/resources/adaptive_imgt_map.tsv` | built by `appendix/build_adaptive_imgt_map.py` from three read-only inputs: (a) the **IMGT gene list + per-allele germline CDR1/CDR2/CDR2.5** in tcrdist3's `db/alphabeta_gammadelta_db.tsv` (locate the env with `python -c "import tcrdist,os;print(os.path.dirname(tcrdist.__file__))"` in conda env `cmp-tcrdist`); (b) the **IMMREP25 release** `~/vcs/manuscripts/2026-immrep25-audit/dump/immrep25/immrep2025_for_release.tsv`, which ships the germline `tcra_cdr1`/`tcra_cdr2`/`tcrb_cdr1`/`tcrb_cdr2` **per record** — this is the validation evidence; (c) the pairSEQ mock cohort `~/vcs/manuscripts/2026-immrep25-audit/results/pairseq_mock.tsv` (extra tokens, no CDRs) | TSV, 163 rows, one per Adaptive token: `chain segment adaptive_token n_records imgt_allele imgt_gene evidence n_candidates cdr1_match cdr2_match allele_ambiguous status` | `python3 appendix/build_adaptive_imgt_map.py --db <tcrdist>/db/alphabeta_gammadelta_db.tsv --immrep <immrep25 release>.tsv --tokens "<pairseq_mock>.tsv:va,ja,vb,jb" --also TCRBD01-01,TCRBD02-01 --out python/vdjtools/resources/adaptive_imgt_map.tsv` (stdlib only) | **DERIVED / computed** — not an experimental measurement. The gene *candidates* come from the IMGT gene list; the *choice* among them is decided by exact-matching germline CDR1+CDR2 amino-acid strings (V) or the mean common-suffix length of the CDR3 3′ end vs the germline J segment, ≥5 aa (J). Status per token: `validated_by_cdr` 138, `gene_list_unique` 16, `tie_resolved` 9, `unresolved_dropped` 0. Cross-check on the 161 record-carrying tokens: **155/161 allele-identical** to tcrdist3's independently hand-curated `db/adaptive_imgt_mapping.csv`; the 6 differences all favour this table (tcrdist3 resolves Adaptive family/tie calls to the *first* family member, and calls `TCRAJ24-01` → `TRAJ24*01` where the records end `…SWGKLQF` = `*02`). NOTE: **Coverage is the 161 tokens observed in those two cohorts** (+2 D-gene tokens injected with `--also`, since `read_immunoseq` maps `D_CALL` but no paired cohort carries a D call), not all of Adaptive's vocabulary. NOTE: Two source-release defects recorded in the `evidence` column, not vdjtools bugs: 120/310 `TCRBV06-05` records carry `TRBV6-1`'s CDRs, and `TCRBV06-X` is genuinely 3 genes. **Purpose**: replaces the regex in `io/convert.py::_adaptive_to_imgt`, which emits a non-existent IMGT name for **100 of these 161 tokens** (22,058 of 44,000 gene calls). Design note + self-check: `appendix/adaptive_imgt_map.md`. **Applied in 3.2.0** — `_adaptive_to_imgt` looks the raw token up here first and falls back to the legacy rewrite off-table |
