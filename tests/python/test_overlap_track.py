...
def test_track_pivots_frequency_across_samples():
    ...
    assert tr.columns == [S.JUNCTION_AA, S.V_CALL, S.J_CALL,
                          "freq_t0", "freq_t1", "freq_sum"]
    # union of clonotypes present in >=1 sample.
    assert set(tr[S.JUNCTION_AA].to_list()) == {"CASSA", "CASSB", "CASSC"}
    row = {r[S.JUNCTION_AA]: r for r in tr.iter_rows(named=True)}
    ...
    assert tr[S.JUNCTION_AA][0] == "CASSA"


def test_track_top_n_and_list_input():
    samples = [_sample(["A", "B", "C"], [100, 10, 1]),
               _sample(["A", "B", "D"], [100, 10, 1])]
    tr = O.track_clonotypes(samples, top=2, key=(S.JUNCTION_AA,))
    assert tr.height == 2                             # top-2 by summed frequency
    assert tr[S.JUNCTION_AA].to_list()[0] == "A"          # most abundant across samples
...
def test_track_empty_inputs():
    ...
    for empty in (O.track_clonotypes({}),
        ...
        assert empty.columns == [S.JUNCTION_AA, S.V_CALL, S.J_CALL]
