...
def test_hamming1_groups_respect_v_and_j():
    ...
    df = pl.DataFrame({
        "cdr3_aa": ["CASSKF", "CASSRF", "CAXXKF", "CASSKF"],
        "v_call":  ["TRBV1",  "TRBV1",  "TRBV1",  "TRBV2"],
        "j_call":  ["TRBJ1",  "TRBJ1",  "TRBJ1",  "TRBJ1"],
    })
    m = metaclonotypes(df, scope="1,0,0,1")
    g = {(r["cdr3_aa"], r["v_call"]): r["meta_id"] for r in m.iter_rows(named=True)}
...
def test_cdr3_only_grouping_ignores_v():
    ...
    df = pl.DataFrame({"cdr3_aa": ["CASSKF", "CASSRF", "CAXXKF"]})
...
def test_1mm_merges_incidence_vs_exact():
    ...
    df = pl.DataFrame({
        ...
        "cdr3_aa": ["CASSKF", "CASSRF", "CASSKF", "CASSRF"],  # KF ~ RF (1 sub)
    ...
    assert "meta_id" in mm.columns and top["cdr3_aa"] in {"CASSKF", "CASSRF"}
