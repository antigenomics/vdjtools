...
def test_downsample_reads_total_and_reproducible():
    ...
    orig = dict(zip(a[S.JUNCTION_AA].to_list(), a[S.COUNT].to_list()))
    for cdr3, c in zip(r1[S.JUNCTION_AA].to_list(), r1[S.COUNT].to_list()):
        assert c <= orig[cdr3]
...
def test_downsample_clones_uniform_without_replacement():
    ...
    orig = dict(zip(a[S.JUNCTION_AA].to_list(), a[S.COUNT].to_list()))
    for cdr3, c in zip(out[S.JUNCTION_AA].to_list(), out[S.COUNT].to_list()):
        assert orig[cdr3] == c
    assert pp.downsample(a, 2, by="clones", seed=3)[S.JUNCTION_AA].to_list() == \
        out[S.JUNCTION_AA].to_list()                              # reproducible


def test_select_top_by_count_renormalizes():
    a = _sample(["CASSA", "CASSB", "CASSC"], [10, 30, 60])
    top = pp.select_top(a, 2)
    assert top[S.JUNCTION_AA].to_list() == ["CASSC", "CASSB"]     # descending by count
