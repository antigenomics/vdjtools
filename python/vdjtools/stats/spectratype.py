...
def _length_frame(df, kind: str, weight: str):
...
def spectratype(df, kind: str = "aa", weight: str = "reads",
                by_locus: bool = True, by=()):
    ...
    Args:
        ...
        by: Extra column(s) to **prepend** to the group key (e.g. ``["sample_id"]`` to
            compute the whole cohort in one grouped pass). Empty by default — output is
            byte-identical to the per-sample spectratype.

    Returns:
        Long-format frame with ``length``, ``weight``, the ``by`` columns and (if
        ``by_locus``) ``locus``, sorted by the group key; a ``pl.LazyFrame`` when
        ``df`` is lazy.
    """
    df = _length_frame(df, kind, weight)
    group = [*by, *([LOCUS, "length"] if by_locus else ["length"])]
    return (df.group_by(group, maintain_order=True)
              .agg(pl.col("weight").sum())
              .sort(group))


def vj_spectratype(df, kind: str = "aa", weight: str = "reads",
                   by_locus: bool = True, keep_allele: bool = False, by=()):
    """CDR3-length distribution broken down by V-J pairing.

    Args:
        df: A clonotype frame (eager or lazy; see :func:`spectratype`).
    ...
    Returns:
        Long-format frame with ``v_call``, ``j_call``, ``length``, ``weight``, the
        ``by`` columns and (if ``by_locus``) ``locus`` (lazy when ``df`` is lazy). A
        V-only spectratype is obtained by summing ``weight`` over ``j_call``.
    """
    df = _length_frame(df, kind, weight)
    ...
    group = [*by, *([LOCUS, V_CALL, J_CALL, "length"] if by_locus
                    else [V_CALL, J_CALL, "length"])]
    return (df.group_by(group, maintain_order=True)
              .agg(pl.col("weight").sum())
              .sort(group))
