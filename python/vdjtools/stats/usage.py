...
def _ensure_locus(df: pl.DataFrame) -> pl.DataFrame:
    return df if LOCUS in df.columns else add_locus(df)


def segment_usage(df: pl.DataFrame, segment: str, weight: str = "reads",
    ...
    Returns:
        Long-format ``pl.DataFrame`` with the segment call column (``v_call`` …), a
        ``weight`` column, and (if ``by_locus``) a ``locus`` column. Rows with a null
        segment call are dropped, so an all-null C gene yields an empty frame.
    ...
    group = [LOCUS, col] if by_locus else [col]
    return (df.group_by(group, maintain_order=True)
              .agg(pl.col("weight").sum())
              .sort(group))


def vj_usage(df: pl.DataFrame, weight: str = "reads", by_locus: bool = True,
    ...
    Returns:
        Long-format ``pl.DataFrame`` with ``v_call``, ``j_call``, ``weight`` and
        (if ``by_locus``) ``locus``. Rows with a null V or J call are dropped.
    """
    df = _ensure_locus(df)
    ...
    group = [LOCUS, V_CALL, J_CALL] if by_locus else [V_CALL, J_CALL]
    return (df.group_by(group, maintain_order=True)
              .agg(pl.col("weight").sum())
              .sort(group))
