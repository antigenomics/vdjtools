...
def _v_options(prep: _Prepared, v: str, s: str) -> list[tuple[int, float]]:
    """(len_v, P(delV|V)) for every 3' trim of V whose germline prefixes ``s``."""
    cut = prep.cut["v"][v]
    ...
    for idx in range(len(cut) + 1):
        ...
        if len_v > len(s):
            continue
        if cut[:len_v] == s[:len_v]:
            p = prep.p_del["v"].get((v, idx - maxp), 0.0)
            if p:
                out.append((len_v, p))
    return out


def _j_options(prep: _Prepared, j: str, s: str) -> list[tuple[int, float]]:
    """(len_j, P(delJ|J)) for every 5' trim of J whose germline suffixes ``s``."""
    cut = prep.cut["j"][j]
    ...
    for idx in range(len(cut) + 1):
        ...
        if len_j > len(s):
            continue
        if contrib == s[len(s) - len_j:]:
            p = prep.p_del["j"].get((j, idx - maxp), 0.0)
            if p:
                out.append((len_j, p))
    return out
...
def pgen_nt(prep: _Prepared, cdr3_nt: str, v: str | None = None, j: str | None = None) -> float:
    ...
    v_mask = [v] if v else prep.functional_v
    j_mask = [j] if j else prep.functional_j
    total = 0.0
    for V in v_mask:
        pv = prep.p_v.get(V, 0.0)
        if pv == 0.0:
            continue
        v_opts = _v_options(prep, V, s)
        if not v_opts:
            continue
        for J in j_mask:
            ...
            if pj == 0.0:
                continue
            j_opts = _j_options(prep, J, s)
            if not j_opts:
                continue
            for len_v, p_dv in v_opts:
                for len_j, p_dj in j_opts:
